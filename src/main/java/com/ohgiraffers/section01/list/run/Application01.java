...
public class Application01 {
    public static void main(String[] args) {
        ...
        *  표준화 된 방법을 제공하는 클래스의 집합을 의미한다.
        *  즉, 데이터는 효율적으로 저장하는 자료구조와 데이터를 처리하는 알고리즘을 미리
        *  인도, 미국 형들이 구현해두었다
        *  */

        /*필기.
        *   Collection Framework 는 크게 3가지 인터페이스 중 한 가지를 상속받아 구현해 두었다.
        *  1. List 인터페이스
        *  2. Set 인터페이스
        *  3. Map 인터페이스
        * */
        ...
        *     - key 를 Set 방식으로 관리하기 때문에 데이터의 순서를 관리하지 않고, 중복된 Ksy 를 허용하지 않는다.
        ...
        *  배열의 단점 : 크기 변경불가, 요소의 추가, 수정, 삭제, 정렬 등이 복잡함
        ...
        *  List 인터페이스 하위의 다양한 구체들로 타입 변경이 가능하기 때문에
        *  레퍼런스 타입은 List 로 해두는게 더 유연한 코드를 작성하는 것이다.*/

        List list = new ArrayList<>();      // 웬만하면 이렇게 쓴다
        
        alist.add("apple");
        alist.add(123); //autoBoxing(값 -> 객체/ lnt -> Integer)
        alist.add(45.67);
        alist.add(new Date());
        ...
        * */
        ...
        System.out.println("alist 의 사이즈 = " + alist.size());
        ...
        /*필기. 원하는 인덱스 위치에 값을 추가하거나 뺄 수 있고 수정할 수도 있다.
...
//        stringList.add(123); 지정한 타입 이외의 인스턴스는 저장하지 못한다.
        ...
        /*필기.
        *   Iterator 란?
        *  Collection ㅇ닌터페이스의 iterator () 메소드르 이용해서 인스턴스를 생성할 수 있다.
        *  hasNext() : 당므요소를 가지고 있으면 true, 더 이상 요소가 없으면 false를 반환
        *  next() : 다음 요소(값)를 반환
        * */

        Iterator<String> dIter = ((LinkedList)stringList).descendingIterator(); // 내림차순이 디센트
